...
import { CollectionsPage} from "@/src/components/sections/CollectionsPage";
...
export default function HTWApp() {
  ...
  return (
    <div style={{ fontFamily: "'Georgia','Times New Roman',serif", background: T.cream, color: T.ink, minHeight: "100vh", overflowX: "hidden" }}>
      {/* Floating WhatsApp */}
      <a href="https://wa.me/2348000000000" target="_blank" rel="noreferrer" className="wa-float" title="Chat with us">💬</a>

      {/* Toast */}
      {toast && (
        <div style={{ position: "fixed", top: 80, right: 20, zIndex: 1001, background: T.sage, color: "#fff", padding: "11px 20px", borderRadius: 9, fontSize: 13, fontWeight: 700, animation: "slideUp .3s ease", boxShadow: "0 4px 20px rgba(59,122,87,.35)" }}>
          ✓ {toast}
        </div>
      )}
      ...
      <main>
        {page === "home"        && <HomePage heroIdx={heroIdx} />}
        {page === "shop"        && <ShopPage />}
        {page === "collections" && <CollectionsPage />}
        {page === "about"       && <AboutPage />}
        {page === "ambassadors" && <AmbassadorsPage />}
